...
const ClientDashboard = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#2D2D2D', fontFamily: 'Jost, sans-serif' }}>
      <style>{`
      ...
      {/* Header */}
      <header style={{ background: '#383838', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', padding: '0 24px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.3rem', color: '#FFF4E1' }}>{firstSalon?.name || 'BeautyTime'}</span>

          <div className="dash-nav-links" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            {firstSalonSlug && (
              <>
                <button onClick={() => navigate(`/s/${firstSalonSlug}/calendar`)} style={{ padding: '8px 14px', background: 'none', border: 'none', color: '#C8B89A', fontSize: '13px', cursor: 'pointer', fontFamily: 'Jost, sans-serif', borderRadius: '8px' }}>Kalendár</button>
                <button onClick={() => navigate('/marketplace')} style={{ padding: '8px 14px', background: 'none', border: 'none', color: '#C8B89A', fontSize: '13px', cursor: 'pointer', fontFamily: 'Jost, sans-serif', borderRadius: '8px' }}>Marketplace</button>
                <button onClick={() => navigate('/notifications')} style={{ padding: '8px 14px', background: 'none', border: 'none', color: '#C8B89A', fontSize: '13px', cursor: 'pointer', fontFamily: 'Jost, sans-serif', borderRadius: '8px' }}>Notifikácie</button>
              </>
            )}
            <div onClick={handleLogout} style={{ width: '36px', height: '36px', borderRadius: '50%', background: C.brown, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600, color: '#FFF4E1', cursor: 'pointer', marginLeft: '8px' }}>
              {initials}
            </div>
          </div>
        ...
        <div className="dash-mobile-menu" style={{ display: 'none', flexDirection: 'column', padding: '16px 24px', gap: '4px', borderTop: `1px solid ${C.border}`, background: '#383838' }}>
          {firstSalonSlug && (
            <>
              <button onClick={() => { navigate(`/s/${firstSalonSlug}/calendar`); setMenuOpen(false); }} style={{ padding: '12px 0', background: 'none', border: 'none', color: '#FFF4E1', fontSize: '14px', cursor: 'pointer', fontFamily: 'Jost, sans-serif', textAlign: 'left', borderBottom: `1px solid ${C.border}` }}>Kalendár</button>
              <button onClick={() => { navigate('/marketplace'); setMenuOpen(false); }} style={{ padding: '12px 0', background: 'none', border: 'none', color: '#FFF4E1', fontSize: '14px', cursor: 'pointer', fontFamily: 'Jost, sans-serif', textAlign: 'left', borderBottom: `1px solid ${C.border}` }}>Marketplace</button>
              <button onClick={() => { navigate('/notifications'); setMenuOpen(false); }} style={{ padding: '12px 0', background: 'none', border: 'none', color: '#FFF4E1', fontSize: '14px', cursor: 'pointer', fontFamily: 'Jost, sans-serif', textAlign: 'left', borderBottom: `1px solid ${C.border}` }}>Notifikácie</button>
            </>
          )}
      ...
      <div className="dash-content" style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2.4rem', color: '#FFF4E1', fontWeight: 400, marginBottom: '4px' }}>{userProfile?.name?.split(' ')[0]}</h1>
          <p style={{ color: '#C8B89A', fontSize: '14px' }}>Váš osobný beauty priestor</p>
        </div>
        ...
        {firstSalon && (
          <div style={{ background: C.brown, borderRadius: '20px', padding: '28px', marginBottom: '24px', position: 'relative', overflow: 'hidden' }}>
            ...
            <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.6rem', color: '#FFF4E1', marginBottom: '4px', fontWeight: 400 }}>{firstSalon.name}</h3>
            <p style={{ fontSize: '13px', color: 'rgba(244,243,238,0.5)', marginBottom: '24px' }}>{firstSalon.address}</p>
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <button onClick={() => navigate(`/book/${firstSalon.id}`)} style={{ padding: '10px 20px', background: '#2D2D2D', color: '#FFF4E1', border: 'none', borderRadius: '10px', fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>Rezervovať</button>
              <button onClick={() => navigate(`/s/${firstSalonSlug}/calendar`)} style={{ padding: '10px 20px', background: 'rgba(244,243,238,0.1)', color: '#FFF4E1', border: '1px solid rgba(244,243,238,0.15)', borderRadius: '10px', fontSize: '11px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>Kalendár</button>
            </div>
          </div>
        )}
        ...
        <div className="dash-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '24px' }}>
          {[
            { label: 'Celkom', value: bookings.length, color: '#FFF4E1' },
            ...
            <div key={stat.label} style={{ background: '#383838', border: `1px solid ${C.border}`, borderRadius: '14px', padding: '16px', textAlign: 'center' }}>
              <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2rem', color: stat.color, marginBottom: '4px' }}>{stat.value}</p>
              <p style={{ fontSize: '10px', color: '#C8B89A', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{stat.label}</p>
            </div>
          ))}
        </div>
        ...
        {repeatBookings.length > 0 && (
          <div style={{ background: '#383838', border: `1px solid ${C.border}`, borderRadius: '16px', padding: '20px 24px', marginBottom: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <p style={{ fontSize: '11px', fontWeight: 500, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C8B89A' }}>Opakované rezervácie</p>
            ...
            {repeatBookings.map(r => (
              <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: '#2D2D2D', borderRadius: '10px', marginBottom: '6px' }}>
                <div>
                  <p style={{ fontSize: '13px', fontWeight: 500, color: '#FFF4E1', fontFamily: 'Jost, sans-serif' }}>{getServiceName(r.serviceId)}</p>
                  <p style={{ fontSize: '11px', color: '#C8B89A', marginTop: '2px' }}>{r.interval === 'weekly' ? 'Každý týždeň' : 'Každý mesiac'} · {r.time}</p>
                </div>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '4px', background: '#383838', borderRadius: '12px', padding: '4px', marginBottom: '20px', border: `1px solid ${C.border}` }}>
        ...
        {loading && <div style={{ textAlign: 'center', padding: '40px' }}><p style={{ color: '#C8B89A' }}>Načítavam...</p></div>}

        {!loading && displayBookings.length === 0 && (
          <div style={{ background: '#383838', border: `1px solid ${C.border}`, borderRadius: '18px', padding: '48px 24px', textAlign: 'center' }}>
            <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.4rem', color: '#FFF4E1', marginBottom: '8px' }}>
              {activeTab === 'upcoming' ? 'Žiadne nadchádzajúce rezervácie' : 'Žiadna história'}
            </p>
            <p style={{ fontSize: '13px', color: '#C8B89A', marginBottom: '20px' }}>
            ...
            {activeTab === 'upcoming' && firstSalon && (
              <button onClick={() => navigate(`/book/${firstSalon.id}`)} style={{ padding: '12px 28px', background: C.brown, color: '#FFF4E1', border: 'none', borderRadius: '10px', fontSize: '11px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>
                Rezervovať teraz
              </button>
            )}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {displayBookings.map(booking => {
            ...
            return (
              <div key={booking.id} style={{ background: '#383838', border: `1px solid ${C.border}`, borderRadius: '16px', padding: '18px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{ fontWeight: 500, color: '#FFF4E1', marginBottom: '3px', fontSize: '14px', fontFamily: 'Jost, sans-serif' }}>{getServiceName(booking.serviceId)}</p>
                      {slotInfo && <p style={{ fontSize: '12px', color: '#C8B89A', marginBottom: '2px' }}>{slotInfo.date} · {slotInfo.time}</p>}
                      <p style={{ fontSize: '12px', color: '#C8B89A' }}>{getSalonName(booking.salonId)}</p>
      ...
      {ratingModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(72,55,47,0.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#383838', borderRadius: '20px', padding: '36px', maxWidth: '420px', width: '100%' }}>
            ...
            <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.6rem', color: '#FFF4E1', marginBottom: '4px', fontWeight: 400 }}>{getServiceName(ratingModal.serviceId)}</h3>
            <p style={{ fontSize: '13px', color: '#C8B89A', marginBottom: '28px' }}>{getSalonName(ratingModal.salonId)}</p>
            ...
            <textarea value={ratingComment} onChange={e => setRatingComment(e.target.value)} placeholder="Váš komentár (nepovinné)..." rows={3}
              style={{ width: '100%', padding: '12px 16px', background: '#2D2D2D', border: `1px solid ${C.border}`, borderRadius: '10px', fontSize: '14px', color: '#FFF4E1', outline: 'none', fontFamily: 'Jost, sans-serif', resize: 'vertical', marginBottom: '20px' }} />
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setRatingModal(null)} style={{ padding: '12px 20px', background: 'transparent', color: '#C8B89A', border: `1px solid ${C.border}`, borderRadius: '10px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>Zrušiť</button>
              <button onClick={handleRate} disabled={submittingRating} style={{ flex: 1, padding: '12px', background: submittingRating ? C.muted : C.brown, color: '#FFF4E1', border: 'none', borderRadius: '10px', fontSize: '12px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: submittingRating ? 'not-allowed' : 'pointer', fontFamily: 'Jost, sans-serif' }}>
      ...
      {showRepeatModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(72,55,47,0.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#383838', borderRadius: '20px', padding: '36px', maxWidth: '420px', width: '100%' }}>
            ...
            <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.6rem', color: '#FFF4E1', marginBottom: '28px', fontWeight: 400 }}>Opakovaná rezervácia</h3>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 500, color: '#C8B89A', marginBottom: '8px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Služba</label>
              <select value={repeatService} onChange={e => setRepeatService(e.target.value)} style={{ width: '100%', padding: '12px 16px', background: '#2D2D2D', border: `1px solid ${C.border}`, borderRadius: '10px', fontSize: '14px', color: '#FFF4E1', outline: 'none', fontFamily: 'Jost, sans-serif' }}>
                <option value=''>Vyber službu...</option>
                {services.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 500, color: '#C8B89A', marginBottom: '8px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Opakovanie</label>
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 500, color: '#C8B89A', marginBottom: '8px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Preferovaný čas</label>
              <input type='time' value={repeatTime} onChange={e => setRepeatTime(e.target.value)} style={{ width: '100%', padding: '12px 16px', background: '#2D2D2D', border: `1px solid ${C.border}`, borderRadius: '10px', fontSize: '14px', color: '#FFF4E1', outline: 'none', fontFamily: 'Jost, sans-serif' }} />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowRepeatModal(false)} style={{ padding: '12px 20px', background: 'transparent', color: '#C8B89A', border: `1px solid ${C.border}`, borderRadius: '10px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>Zrušiť</button>
              <button onClick={handleSaveRepeat} disabled={savingRepeat} style={{ flex: 1, padding: '12px', background: savingRepeat ? C.muted : C.brown, color: '#FFF4E1', border: 'none', borderRadius: '10px', fontSize: '12px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: savingRepeat ? 'not-allowed' : 'pointer', fontFamily: 'Jost, sans-serif' }}>
