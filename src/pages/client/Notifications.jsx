...
const Notifications = () => {
  ...
  const Toggle = ({ value, onChange }) => (
    <div onClick={onChange} style={{ width: '52px', height: '28px', borderRadius: '14px', background: value ? '#DFA0AA' : 'rgba(90,60,40,0.18)', position: 'relative', cursor: 'pointer', transition: 'background 0.2s', flexShrink: 0 }}>
    ...
    <div style={{ minHeight: '100vh', background: '#E4E0CC' }}>
      ...
      <div style={{ maxWidth: '560px', margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ marginBottom: '40px' }}>
          ...
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2rem', color: '#2A1A10', marginBottom: '8px' }}>Last Minute notifikácie</h2>
        ...
        {loading ? <div style={{ textAlign: 'center', padding: '40px' }}><div className="spinner" style={{ margin: '0 auto' }} /></div> : (
          <>
            <div style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                <p style={{ fontWeight: 500, color: '#2A1A10', marginBottom: '4px', fontFamily: 'Jost, sans-serif' }}>Povoliť notifikácie</p>
            ...
            {enabled && (
              <div style={{ ...cardStyle, marginBottom: '20px' }}>
                ...
                <div onClick={toggleAll} style={{ padding: '14px 18px', borderRadius: '12px', border: `1.5px solid ${allServices ? '#DFA0AA' : 'rgba(90,60,40,0.18)'}`, background: allServices ? 'rgba(106,93,82,0.06)' : '#E4E0CC', cursor: 'pointer', marginBottom: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <p style={{ fontWeight: 500, color: '#2A1A10', fontSize: '14px', fontFamily: 'Jost, sans-serif' }}>Všetky služby</p>
                    <p style={{ fontSize: '12px', color: '#845F4A', marginTop: '2px' }}>Upozorni ma na akýkoľvek last minute termín</p>
                  </div>
                  <div style={{ width: '22px', height: '22px', borderRadius: '50%', border: `2px solid ${allServices ? '#DFA0AA' : '#8A7260'}`, background: allServices ? '#DFA0AA' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {allServices && <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#E4E0CC' }} />}
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {services.map(service => {
                    ...
                    return (
                      <div key={service.id} onClick={() => toggleService(service.id)} style={{ padding: '14px 18px', borderRadius: '12px', border: `1.5px solid ${isSel ? '#DFA0AA' : 'rgba(90,60,40,0.18)'}`, background: isSel ? 'rgba(106,93,82,0.06)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                          <p style={{ fontWeight: 500, color: '#2A1A10', fontSize: '14px', fontFamily: 'Jost, sans-serif' }}>{service.name}</p>
                          <p style={{ fontSize: '12px', color: '#845F4A', marginTop: '2px' }}>{service.duration} min · {service.price} €</p>
                        </div>
                        <div style={{ width: '22px', height: '22px', borderRadius: '6px', border: `2px solid ${isSel ? '#DFA0AA' : '#8A7260'}`, background: isSel ? '#DFA0AA' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          {isSel && <span style={{ color: '#E4E0CC', fontSize: '13px', lineHeight: 1 }}>✓</span>}
                        </div>
                      </div>
                    );
                  })}
            ...
            <button onClick={handleSave} disabled={saving} style={{ width: '100%', padding: '15px', background: saving ? '#8A7260' : '#DFA0AA', color: '#E4E0CC', border: 'none', borderRadius: '14px', fontSize: '12px', fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'Jost, sans-serif' }}>
