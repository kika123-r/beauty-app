  ...
  return (
    <div style={{ minHeight: '100vh', background: '#E8E4D0' }}>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '32px 20px' }}>
        ...
        {/* Filtre */}
        <div style={{ background: '#FAFAF5', border: '1px solid #E2E2DE', borderRadius: '14px', padding: '4px', marginBottom: '12px', display: 'flex', boxShadow: '0 2px 12px rgba(28,28,27,0.04)' }}>
          ...
          {salons.map(salon => {
            ...
            return (
              <div key={salon.id} onClick={() => setSelectedSalonDetail(selectedSalonDetail === salon.id ? null : salon.id)}
                style={{ background: '#FAFAF5', border: '1px solid #E2E2DE', borderRadius: '16px', padding: '16px 20px', cursor: 'pointer', boxShadow: '0 2px 12px rgba(28,28,27,0.04)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <span style={{ fontSize: '11px', color: isOpen ? '#7A9E7E' : '#DFA0AA', fontWeight: 500 }}>{isOpen ? `Otvorené · ${hours.open}–${hours.close}` : 'Zatvorené'}</span>
                    </div>
                  </div>
                  <span style={{ fontSize: '12px', color: '#845F4A' }}>{selectedSalonDetail === salon.id ? '▲' : '▼'}</span>
                </div>
                {selectedSalonDetail === salon.id && (
                  <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #F5F0EA' }}>
                    {salon.description && <p style={{ fontSize: '13px', color: '#845F4A', lineHeight: 1.6, marginBottom: '12px' }}>{salon.description}</p>}
                    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                      {salon.phone && <a href={`tel:${salon.phone}`} style={{ fontSize: '12px', color: '#DFA0AA', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>📞 {salon.phone}</a>}
                      {salon.email && <a href={`mailto:${salon.email}`} style={{ fontSize: '12px', color: '#DFA0AA', textDecoration: 'none' }}>✉️ {salon.email}</a>}
                      {salon.instagram && <a href={`https://instagram.com/${salon.instagram.replace('@','')}`} target="_blank" rel="noreferrer" style={{ fontSize: '12px', color: '#DFA0AA', textDecoration: 'none' }}>Instagram {salon.instagram}</a>}
                      {salon.website && <a href={salon.website.startsWith('http') ? salon.website : `https://${salon.website}`} target="_blank" rel="noreferrer" style={{ fontSize: '12px', color: '#DFA0AA', textDecoration: 'none' }}>🌐 Web</a>}
                    </div>
                  </div>
                )}
              </div>
        ...
        {salons.length > 1 && (
          <div style={{ background: '#FAFAF5', border: '1px solid #E2E2DE', borderRadius: '14px', padding: '4px', marginBottom: '24px', display: 'flex', gap: '4px', flexWrap: 'wrap', boxShadow: '0 2px 12px rgba(28,28,27,0.04)' }}>
