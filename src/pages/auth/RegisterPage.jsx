      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            ...
            <h1 style={{ fontSize: '1.8rem', color: '#E8F0F8', marginBottom: '6px', fontFamily: 'Cormorant Garamond, serif' }}>BeautyTime</h1>
            <p style={{ fontSize: '13px', color: '#7691AD' }}>Vytvor si nový účet</p>
          </div>
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '10px', fontWeight: 500, color: '#7691AD', marginBottom: '8px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Meno a priezvisko</label>
              <input type="text" name="name" placeholder="Jana Nováková" value={form.name} onChange={handleChange}
                style={{ width: '100%', padding: '14px 18px', background: '#142F52', border: '1px solid #E2E2DE', borderRadius: '12px', fontSize: '14px', color: '#E8F0F8', outline: 'none', fontFamily: 'Jost, sans-serif', boxSizing: 'border-box' }}
                onFocus={(e) => e.target.style.borderColor = '#FF929A'} onBlur={(e) => e.target.style.borderColor = 'rgba(185,207,221,0.12)'} />
            </div>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '10px', fontWeight: 500, color: '#7691AD', marginBottom: '8px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Email</label>
              <input type="email" name="email" placeholder="tvoj@email.com" value={form.email} onChange={handleChange}
                style={{ width: '100%', padding: '14px 18px', background: '#142F52', border: '1px solid #E2E2DE', borderRadius: '12px', fontSize: '14px', color: '#E8F0F8', outline: 'none', fontFamily: 'Jost, sans-serif', boxSizing: 'border-box' }}
                onFocus={(e) => e.target.style.borderColor = '#FF929A'} onBlur={(e) => e.target.style.borderColor = 'rgba(185,207,221,0.12)'} />
            </div>
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', fontSize: '10px', fontWeight: 500, color: '#7691AD', marginBottom: '8px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Heslo</label>
            ...
            <button type="submit" disabled={loading} style={{ width: '100%', padding: '15px', background: loading ? '#53728A' : '#FF929A', color: '#0A1F36', border: 'none', borderRadius: '12px', fontSize: '12px', fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Jost, sans-serif' }}>
              {loading ? 'Vytváram účet...' : 'Zaregistrovať sa'}
            </button>
          </form>
          <p style={{ textAlign: 'center', marginTop: '28px', fontSize: '13px', color: '#7691AD' }}>
            Už máš účet?{' '}
            <Link to={ROUTES.LOGIN} style={{ color: '#FF929A', fontWeight: 500 }}>Prihlás sa</Link>
          </p>
        </div>
      </div>
    </div>
