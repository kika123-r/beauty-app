            ...
            {workers.map(worker => (
              <div key={worker.id} style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={() => handleDelete(worker.id)} style={{ padding: '7px 14px', background: 'transparent', color: '#C8A882', border: '1px solid #8B3A3A', borderRadius: '8px', fontSize: '11px', fontWeight: 500, cursor: 'pointer', fontFamily: 'Jost, sans-serif', textTransform: 'uppercase' }}>Vymazať</button>
                </div>
              </div>
            ))}
          </div>
