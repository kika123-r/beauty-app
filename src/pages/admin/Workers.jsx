      ...
      <div style={{ maxWidth: '700px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px' }}>
          <div>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2rem', color: '#1C1C1B' }}>Pracovníci</h2>
          </div>
          {!showForm && (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: imagePreview ? 'transparent' : 'linear-gradient(135deg, #D4C5B0, #B7AC9B)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {imagePreview ? <img src={imagePreview} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.5rem', color: '#F5F0EA' }}>{form.name?.charAt(0) || '?'}</span>}
                </div>
                <div>
                  <label style={labelStyle}>Fotografia</label>
                  <input type="file" accept="image/*" onChange={handleImageChange} style={{ fontSize: '13px', color: '#6A5D52', fontFamily: 'Jost, sans-serif' }} />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                ...
                <div>
                  ...
                  <input type="text" value={form.position} onChange={e => setForm({...form, position: e.target.value})} placeholder="Kaderníčka" style={inputStyle} onFocus={e => e.target.style.borderColor='#6A5D52'} onBlur={e => e.target.style.borderColor='#E2E2DE'} />
                </div>
              </div>

              <div style={{ marginBottom: '16px' }}>
                <label style={labelStyle}>Bio</label>
                <textarea value={form.bio} onChange={e => setForm({...form, bio: e.target.value})} placeholder="Krátky popis..." rows={2} style={{ ...inputStyle, resize: 'vertical' }} onFocus={e => e.target.style.borderColor='#6A5D52'} onBlur={e => e.target.style.borderColor='#E2E2DE'} />
              </div>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {services.map(s => (
                    <div key={s.id} onClick={() => toggleService(s.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: form.serviceIds.includes(s.id) ? 'rgba(106,93,82,0.06)' : '#F5F0EA', border: `1px solid ${form.serviceIds.includes(s.id) ? '#6A5D52' : '#E2E2DE'}`, borderRadius: '10px', cursor: 'pointer' }}>
                      <div style={{ width: '18px', height: '18px', borderRadius: '4px', border: `2px solid ${form.serviceIds.includes(s.id) ? '#6A5D52' : '#D4C5B0'}`, background: form.serviceIds.includes(s.id) ? '#6A5D52' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        {form.serviceIds.includes(s.id) && <span style={{ color: '#F5F0EA', fontSize: '11px' }}>✓</span>}
                      </div>
                      <div>
                        <p style={{ fontSize: '13px', fontWeight: 500, color: '#1C1C1B', fontFamily: 'Jost, sans-serif' }}>{s.name}</p>
                        <p style={{ fontSize: '11px', color: '#979086' }}>{s.duration} min · {s.price} €</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button type="submit" disabled={loading || uploading} style={{ padding: '12px 24px', background: loading ? '#B7AC9B' : '#6A5D52', color: '#F5F0EA', border: 'none', borderRadius: '12px', fontSize: '12px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>
                  {uploading ? 'Nahrávam...' : loading ? 'Ukladám...' : editId ? 'Uložiť' : 'Pridať pracovníka'}
                </button>
                <button type="button" onClick={() => { setShowForm(false); setEditId(null); setForm({ name: '', position: '', serviceIds: [], bio: '', photoUrl: '' }); setImageFile(null); setImagePreview(null); }} style={{ padding: '12px 24px', background: 'transparent', color: '#979086', border: '1px solid #E2E2DE', borderRadius: '12px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>
                  Zrušiť
                </button>
              </div>
            </form>
        ...
        {workers.length === 0 ? (
          <div style={{ ...cardStyle, textAlign: 'center', padding: '60px' }}>
            <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.3rem', color: '#1C1C1B', marginBottom: '8px' }}>Zatiaľ žiadni pracovníci</p>
            <p style={{ fontSize: '13px', color: '#979086' }}>Pridajte pracovníkov a priraďte im služby.</p>
          </div>
        ) : (
              ...
              <div key={worker.id} style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, #D4C5B0, #B7AC9B)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {worker.photoUrl ? <img src={worker.photoUrl} alt={worker.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.2rem', color: '#F5F0EA' }}>{worker.name?.charAt(0)}</span>}
                  </div>
                  <div>
                    <p style={{ fontWeight: 500, color: '#1C1C1B', fontSize: '14px', fontFamily: 'Jost, sans-serif', marginBottom: '2px' }}>{worker.name}</p>
                    <p style={{ fontSize: '12px', color: '#6A5D52', marginBottom: '4px' }}>{worker.position}</p>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      {(worker.serviceIds || []).map(sid => {
                        const s = services.find(sv => sv.id === sid);
                        return s ? <span key={sid} style={{ fontSize: '10px', color: '#979086', background: '#F5F0EA', padding: '2px 8px', borderRadius: '8px' }}>{s.name}</span> : null;
                      })}
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => handleEdit(worker)} style={{ padding: '7px 14px', background: 'transparent', color: '#6A5D52', border: '1px solid #E2E2DE', borderRadius: '8px', fontSize: '11px', fontWeight: 500, cursor: 'pointer', fontFamily: 'Jost, sans-serif', textTransform: 'uppercase' }}>Upraviť</button>
