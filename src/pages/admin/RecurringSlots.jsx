              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {form.times.map((time, i) => (
                  <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    {form.times.length > 1 && (
                      <button type="button" onClick={() => removeTime(i)} style={{ padding: '8px 12px', background: 'transparent', color: '#DFA0AA', border: '1px solid #8B3A3A', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>×</button>
                    )}
                  </div>
                ))}
                ...
                <div key={tmpl.id} style={cardStyle}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                      ...
                      <button onClick={() => handleDelete(tmpl.id)} style={{ padding: '9px 14px', background: 'transparent', color: '#DFA0AA', border: '1px solid #8B3A3A', borderRadius: '10px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>Vymazať</button>
                    </div>
                  </div>
                </div>
              );
