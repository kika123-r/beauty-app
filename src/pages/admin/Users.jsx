...
const Users = () => {
  ...
  const getScoreColor = (score) => score >= RELIABILITY.WARNING_THRESHOLD ? '#7A9E7E' : score >= RELIABILITY.BLOCK_THRESHOLD ? '#D4A85A' : '#DFA0AA';
            ...
            return (
              <div key={user.uid} style={cardStyle}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        ...
                        {user.blocked && <span style={{ fontSize: '10px', color: '#DFA0AA', background: 'rgba(139,58,58,0.1)', padding: '2px 8px', borderRadius: '20px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Blokovaný</span>}
                      ...
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                        {[
                          ...
                          { label: 'No-show', value: noShows, color: noShows > 0 ? '#DFA0AA' : 'var(--text-faint)', bg: noShows > 0 ? 'rgba(139,58,58,0.08)' : 'var(--bg-elevated)' },
                  ...
                  <button onClick={() => handleBlock(user)} style={{ padding: '8px 16px', background: 'transparent', color: user.blocked ? 'var(--primary)' : '#DFA0AA', border: `1px solid ${user.blocked ? 'var(--border)' : '#DFA0AA'}`, borderRadius: '10px', fontSize: '11px', fontWeight: 500, cursor: 'pointer', fontFamily: 'Jost, sans-serif', letterSpacing: '0.06em', textTransform: 'uppercase', flexShrink: 0, marginLeft: '16px' }}>
