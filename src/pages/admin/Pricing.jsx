    ...
    <AdminLayout>
      <div style={{ maxWidth: '900px' }}>
        <div style={{ marginBottom: '36px' }}>
          <p style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#979086', marginBottom: '8px' }}>Cenové plány</p>
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2rem', color: '#1C1C1B' }}>Vyberte si plán</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '40px' }}>
          {tiers.map((tier) => {
            ...
            return (
              <div key={tier} style={{ background: '#FFFFFF', border: `${isPro ? '2px' : '1px'} solid ${isPro ? '#6A5D52' : '#E2E2DE'}`, borderRadius: '20px', padding: '24px 20px', position: 'relative', boxShadow: isPro ? '0 8px 32px rgba(106,93,82,0.12)' : '0 2px 12px rgba(28,28,27,0.04)' }}>
                {isPro && (
                  <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: '#6A5D52', color: '#F5F0EA', fontSize: '10px', fontWeight: 500, letterSpacing: '0.1em', padding: '4px 14px', borderRadius: '20px', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                    Odporúčame
                  </div>
                )}
                <p style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#979086', marginBottom: '8px' }}>{config.name}</p>
                ...
                {config.price > 0 && <p style={{ fontSize: '11px', color: '#979086', marginBottom: '16px' }}>/ mesiac</p>}
                ...
                <button
                  ...
                  style={{ width: '100%', padding: '11px', background: isCurrent ? '#F5F0EA' : isPro ? '#6A5D52' : 'transparent', color: isCurrent ? '#979086' : isPro ? '#F5F0EA' : '#6A5D52', border: `1px solid ${isCurrent ? '#E2E2DE' : '#6A5D52'}`, borderRadius: '10px', fontSize: '11px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: isCurrent ? 'default' : 'pointer', fontFamily: 'Jost, sans-serif' }}
                >
                ...
                {!isCurrent && tier !== TIERS.FREE && (
                  <p style={{ fontSize: '10px', color: '#979086', textAlign: 'center', marginTop: '8px' }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E2E2DE', borderRadius: '20px', overflow: 'hidden' }}>
          ...
          {FEATURES.map((feature, i) => (
            <div key={feature.key} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '14px 24px', background: i % 2 === 0 ? '#FFFFFF' : '#FAFAF8', borderBottom: i < FEATURES.length - 1 ? '1px solid #F5F0EA' : 'none' }}>
