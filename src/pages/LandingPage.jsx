  ...
  return (
    <div>
          ...
          {PLANS.map((plan, i) => {
            ...
            return (
              <div key={plan.name} className="plan-card"
                ...
                {plan.highlight && (
                  <div style={{ display: 'inline-block', background: '#2D2D2D', color: C.accent, fontSize: '10px', fontWeight: 700, letterSpacing: '0.2em', padding: '4px 14px', borderRadius: '999px', textTransform: 'uppercase', marginBottom: '24px', fontFamily: 'Julius Sans One, sans-serif' }}>
                    Odporúčame
                  </div>
                )}

                <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: mutedColor, marginBottom: '12px', fontFamily: 'Julius Sans One, sans-serif' }}>{plan.name}</p>
                ...
                {plan.price !== '0' && <p style={{ fontSize: '13px', color: mutedColor, marginBottom: '0', fontFamily: 'Julius Sans One, sans-serif' }}>/ mesiac</p>}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
                  {plan.features.map((f, j) => (
                    <div key={j} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ fontSize: '13px', color: mutedColor, fontFamily: 'Julius Sans One, sans-serif', fontWeight: 400 }}>{f}</span>
                    </div>
                  ))}
                </div>
                ...
                {isActive && (
                  <button onClick={e => { e.stopPropagation(); navigate('/beauty-admin-portal-2024'); }}
                    style={{ width: '100%', padding: '13px', background: plan.highlight ? '#2D2D2D' : C.accent, color: plan.highlight ? C.accent : '#2D2D2D', border: 'none', fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Julius Sans One, sans-serif', fontWeight: 700, borderRadius: '6px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px', marginTop: '36px' }}>
        ...
        <p style={{ fontSize: '12px', color: C.faint, fontFamily: 'Julius Sans One, sans-serif', marginLeft: 'auto' }}>
...
const LandingPage = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Julius Sans One, sans-serif', color: C.milk }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Julius+Sans+One:ital,wght@0,200;0,300;0,400;0,500;0,600;0,700;1,300&family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;1,300;1,400&display=swap');
      ...
      <nav style={{ background: C.bg, borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 40px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div className="nav-links" style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
            ...
            <button onClick={() => navigate('/login')} style={{ fontSize: '11px', letterSpacing: '0.18em', color: C.muted, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'Julius Sans One, sans-serif', textTransform: 'uppercase', fontWeight: 500 }}>Prihlásiť sa</button>
            <button onClick={() => navigate('/beauty-admin-portal-2024')} style={{ fontSize: '11px', letterSpacing: '0.18em', background: C.accent, color: C.bg, border: 'none', padding: '10px 22px', cursor: 'pointer', fontFamily: 'Julius Sans One, sans-serif', textTransform: 'uppercase', fontWeight: 700, borderRadius: '4px' }}>Začať zadarmo</button>
          </div>
        </div>
      </nav>
      ...
      <section style={{ maxWidth: '900px', margin: '0 auto', padding: '110px 40px 90px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/beauty-admin-portal-2024')} style={{ background: C.accent, color: C.bg, border: 'none', padding: '16px 44px', fontSize: '12px', letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Julius Sans One, sans-serif', fontWeight: 700, borderRadius: '4px' }}>Vyskúšať zadarmo</button>
          <button onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })} style={{ background: 'transparent', color: C.muted, border: `1px solid ${C.border}`, padding: '16px 32px', fontSize: '12px', letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Julius Sans One, sans-serif', fontWeight: 500, borderRadius: '4px' }}>Zobraziť ceny</button>
        </div>
      ...
      <section style={{ background: '#1A1A1A', padding: '90px 40px', textAlign: 'center', borderTop: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <button onClick={() => navigate('/beauty-admin-portal-2024')} style={{ background: C.accent, color: C.bg, border: 'none', padding: '16px 48px', fontSize: '12px', letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Julius Sans One, sans-serif', fontWeight: 700, borderRadius: '4px' }}>
      ...
      <footer style={{ background: '#1A1A1A', borderTop: `1px solid ${C.border}`, padding: '24px 40px', textAlign: 'center' }}>
        <p style={{ fontSize: '12px', color: C.faint, letterSpacing: '0.1em', fontFamily: 'Julius Sans One, sans-serif' }}>© 2025 BeautyTime</p>
      </footer>
    </div>
  );
};
